  ...
  class ApiClient {
    constructor(token, options = {}, webhookReplyEnvelope = {}) {
      ...
      this.call = async (method, p, signal) => {
        ...
        const successPromise = (0, shim_node_js_1.fetch)(url instanceof URL ? url.href : url, options2).catch((0, error_js_1.toHttpError)(method, opts.sensitiveLogs));
...
var envPaths = [resolve(process.cwd(), ".env"), resolve(process.cwd(), "../.env")];
...
var REQUIRED_TAG = (process.env.REQUIRED_TAG ?? "#анонс_группы").toLowerCase();
var FORMAT_LINK = process.env.FORMAT_LINK ?? "https://t.me";
var SPAM_WINDOW_MS = parsePositiveInt(process.env.SPAM_WINDOW_MS, 15000);
var SPAM_MAX_MESSAGES = parsePositiveInt(process.env.SPAM_MAX_MESSAGES, 3);
var DUPLICATE_WINDOW_MS = parsePositiveInt(process.env.DUPLICATE_WINDOW_MS, 30000);
if (!BOT_TOKEN) {
...
var spamBuckets = new Map;
var lastContentByUser = new Map;
var lastPinnedByChat = new Map;
var announcementsSessionStore = new Map;
var SERVICE_MESSAGE_KEYS = [
  "new_chat_members",
  "left_chat_member",
  "new_chat_title",
  "new_chat_photo",
...
function hitRateLimit(chatId, userId, now) {
...
function isDuplicate(chatId, userId, normalizedText, now) {
  if (!normalizedText) {
    return false;
  }
  const key = `${chatId}:${userId}`;
...
function normalizeForChecks(text) {
